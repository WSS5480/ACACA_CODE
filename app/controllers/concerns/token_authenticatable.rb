...
module TokenAuthenticatable
  ...
  def authenticate_entity!
    Rails.logger.debug "=== 🔐 authenticate_entity! START ==="
    Rails.logger.debug "🔐 Request path: #{request.path}"
    Rails.logger.debug "🔐 Request method: #{request.method}"
    Rails.logger.debug "🔐 Request origin: #{request.headers['Origin']}"
    
    auth_header = request.headers['Authorization']
    Rails.logger.debug "🔐 Authorization header present: #{auth_header.present?}"
    Rails.logger.debug "🔐 Authorization header preview: #{auth_header ? auth_header[0..50] + '...' : 'N/A'}"
    ...
    if auth_header_split&.first == 'Bearer'
      ...
      Rails.logger.debug "🔐 Bearer token extracted, length: #{token&.length}"
    else
    ...
    Rails.logger.debug "🔐 Proceeding to authenticate user from token"
    authenticate_user_from_token(token)
    Rails.logger.debug "=== 🔐 authenticate_entity! END ==="
  end

  def authenticate_user_from_token(token)
    Rails.logger.debug "=== 🔐 authenticate_user_from_token START ==="
    Rails.logger.debug "🔐 Token length: #{token.length}"
    Rails.logger.debug "🔐 Token preview: #{token[0..20]}..."
    
    begin
      # Use the same secret key as Devise-JWT
      secret_key = ENV.fetch('DEVISE_JWT_SECRET_KEY', Rails.application.secret_key_base)
      Rails.logger.debug "🔐 Using secret key (present): #{secret_key.present?}"
      
      payload = JWT.decode(token, secret_key, true, { algorithm: 'HS256' }).first
      Rails.logger.debug "🔐 JWT payload decoded successfully"
      Rails.logger.debug "🔐 Payload keys: #{payload.keys}"
      Rails.logger.debug "🔐 User ID from payload: #{payload['sub']}"
      Rails.logger.debug "🔐 JTI from payload: #{payload['jti']}"
      Rails.logger.debug "🔐 Expiration: #{Time.at(payload['exp']) if payload['exp']}"
      
      # Check if token is in denylist
      jti = payload['jti']
      Rails.logger.debug "🔐 Checking if JTI is in denylist: #{jti}"
      ...
      Rails.logger.debug "✅ Token is not in denylist"
      Rails.logger.debug "🔐 Looking for user with ID: #{payload['sub']}"
    ...
    Rails.logger.debug "=== 🔐 authenticate_user_from_token END ==="
  end
end
